...
[Path to the spec being worked on, e.g. specs/<feature>.md, and its status]
...
Read `memory.md` first. Then, if the memory names an active spec, read `specs/<feature>.md`. Then check for these specific context files if they exist and read only those:
