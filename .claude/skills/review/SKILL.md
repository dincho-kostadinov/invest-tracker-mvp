...
- The spec for this feature at `specs/<feature>.md` (written by `/architect`) — the primary benchmark
...
If no spec exists, ask the developer to describe what the feature was supposed to do before reviewing. You cannot verify correctness without knowing what correct looks like. Layer 1 below checks the build against the spec's "Done when" checklist.
